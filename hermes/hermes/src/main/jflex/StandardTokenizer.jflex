...
ALPHANUM  = ({ALPHA}|[:digit:])+"'"?("-"?({ALPHA}|[:digit:]))*
...
ACRONYM = ( [A-Z]("."{ALPHANUM}+)+"."? ) | ( ([A-Z]|[:digit:])*[A-Z]+([A-Z]|[:digit:])* ([A-Z]|[:digit:])* )
...
<YYINITIAL>{
 ...
 {NUMBER}([-/]*{NUMBER})*  {return attachToken(TokenType.NUMBER);}
 {NUMBER}/{AMPM}        {return attachToken(TokenType.NUMBER);}
