...
enum CaduceusParser implements TokenDef {
    ...
    public static CaduceusProgram parse(Resource resource) throws ParseException, IOException {
        ...
        while (ts.hasNext()) {
            ...
            } else if (token.isInstance(IMPORT)) {
                String rulesFile = token.getText().replaceAll("@import", "").strip();
                CaduceusProgram tmp = CaduceusProgram.read(resource.getParent().getChild(rulesFile));
                for (Rule rule1 : tmp.rules) {
                    for (RelationProvider relationProvider : rule1.getRelationProviders()) {
                        rule.relationProvider(relationProvider);
                    }
                }
            }
        }
