...
public interface HString extends Span, StringLike, Serializable {
   ...
   static HString toHString(Object o) {
      ...
      if (o instanceof Collection<?>) {
         return HString.union(Cast.<Iterable<HString>>as(o));
      }
    ...
    * @return {@code true} if the given annotation type is a subtype of the given annotation type, {@code false}
    * otherwise
    */
   ...
   default List<Frame> getFrames(@NonNull AnnotationType frameType, @NonNull RelationType frameRoleType) {
      return annotations(frameType).stream().map(trigger -> Frame.from(trigger, frameRoleType))
                                   .collect(Collectors.toList());
   }
   ...
   default List<Relation> outgoingRelations(boolean includeSubAnnotations) {
      return outgoingRelationStream(includeSubAnnotations).distinct().collect(Collectors.toList());
   }
   ...
   /**
    * Assumes the HString only overlaps with a single sentence and returns it. This is equivalent to calling
    * {@link #first(AnnotationType)} with the annotation type set to <code>Types.SENTENCE</code>
    *
   ...
   /**
    * Generates a unique span id for this HString that uniquely identifies it on the document.
    *
    * @return The span id
    */
   default String spanId() {
      String spanId = document().getId() + "::F::" + start() + "::" + end();
      if (isAnnotation()) {
         Annotation a = asAnnotation();
         if (!a.isDetached()) {
            spanId = document().getId() + "::A::" + a.getId();
         }
      }
      return spanId;
   }
