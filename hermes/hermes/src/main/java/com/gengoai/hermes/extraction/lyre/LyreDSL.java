...
public final class LyreDSL {
    ...
    public static LyreExpression isA(@NonNull LyreExpression type) {
        return isA($_, type);
    }

    public static LyreExpression isA(@NonNull LyreExpression expression, @NonNull LyreExpression type) {
        Validation.checkArgument(expression.isInstance(HSTRING),
                                 "Illegal Expression: isAlphaNumeric only accepts a HSTRING but '"
                                         + expression + "' was provided which is of type " + expression.getType());
        return new LyreExpression(formatMethod("isA", expression, type),
                                  PREDICATE,
                                  o -> processPred(expression.applyAsObject(o),
                                                   a -> Optional.ofNullable(a)
                                                                .map(HString::toHString)
                                                                .map(HString::asAnnotation)
                                                                .map(h -> h.getType() == AnnotationType.valueOf(type.applyAsString(h)))
                                                                .orElse(false)));
    }
