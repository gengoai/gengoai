...
public class ENWordSenseAnnotator extends SentenceLevelAnnotator {
                        ...
                        for (String previous : working) {
                            for (String next : getAllLemmas(token, lemmatizer)) {
                                ...
                                if (lemmas.containsKey(phrase)) {
                                    nextSet.add(phrase);
                                    matched = true;
                                } else if (!lemmas.prefix(phrase).isEmpty()) {
                                    nextSet.add(phrase);
                                }
                            }
                        }
    ...
    private Annotation createAnnotation(Document document, Span span) {
        Annotation annotation = document.annotationBuilder(Types.WORD_SENSE)
                                        .bounds(span)
                                        .createDetached();
        List<Sense> senses = WordNet.getInstance().getSenses(span.toString(),
                                                             PartOfSpeech.forText(annotation),
                                                             document.getLanguage());
        if (senses.isEmpty()) {
            senses = WordNet.getInstance().getSenses(span.toString(),
                                                     PartOfSpeech.ANY,
                                                     document.getLanguage());
        }
        if (senses.isEmpty()) {
        ...
        annotation.attach();
        senses.sort(Comparator.comparingInt(Sense::getSenseNumber));
