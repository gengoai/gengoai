...
public class SimpleDocumentView extends JPanel {
    ...
    public SimpleDocumentView(final Document document) {
        ...
        var tblAttributes = with(new MangoTable($("Attribute", String.class),
                                                $("Value", String.class)), $ -> {
            ...
            $.withColumn(0, c -> c.setMaxWidth(250));
        });
        tabBottomTools.addTab("Attributes", new JScrollPane(tblAttributes));
        document.attributeMap().forEach((k, v) -> tblAttributes.addRow(k.label(), v.toString()));
