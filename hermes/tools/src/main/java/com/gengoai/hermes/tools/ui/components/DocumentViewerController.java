...
final class DocumentViewerController {
    ...
    protected void onDocumentViewSelectionChange(SelectionChangeEvent e) {
        if (e.getNewSelection() != null) {
            ...
            Point sp = _viewer.documentView.modelToView(start);
            Point loc = _viewer.getParent().getLocation();
            SwingUtilities.convertPointToScreen(loc, _viewer.getParent());
            _viewer.dlgFloatingWindow.setLocation(new Point(loc.x + sp.x, loc.y + sp.y));
            _viewer.dlgFloatingWindow.setSelection(start, end, _viewer.documentView.getBestMatchingStyledSpan(start, end));
            //            dlgFloatingWindow.setMinimumSize(new Dimension(100, 28));
            _viewer.dlgFloatingWindow.setVisible(true);
            ...
            _viewer.dlgFloatingWindow.setVisible(false);
        }
    }
