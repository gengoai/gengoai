...
public abstract class LuceneIndex implements AutoCloseable, Iterable<IndexDocument> {
    ...
    public void addOrReplace(@NonNull String primaryKeyField, @NonNull IndexDocument... documents) throws IOException {
        addOrReplace(primaryKeyField, Arrays.asList(documents));
    }

    public void addOrReplace(@NonNull String primaryKeyField, @NonNull Iterable<? extends IndexDocument> documents) throws IOException {
        try (LuceneIndexWriter writer = writer()) {
            for (IndexDocument document : documents) {
                String primaryKey = document.get(primaryKeyField).asString();
                writer.updateDocument(new Term(primaryKeyField, primaryKey), document.asIndexableFields(config));
            }
        }
    }
     ...
     *
     * @param idField    the field containing the document id
