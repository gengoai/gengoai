...
embed = GloveEmbedding(dimension=word_dimensions,
                       ...
                       mask_zero=True)
translate = K.layers.TimeDistributed(K.layers.Dense(word_dimensions, activation='relu'), name="word_embedding")

premise = translate(embed(input_layers["w1"]))
hypothesis = translate(embed(input_layers["w2"]))

# Sentence Embeddings
sentence_embed = K.layers.Lambda(lambda x: K.backend.sum(x, axis=1), output_shape=(sentence_hidden_size,))
...
for i in range(3):
    out = K.layers.Dense(2 * sentence_hidden_size, activation="relu", W_regularizer=l2(4e-6))(out)
    out = K.layers.Dropout(DP)(out)
...
model.fit(x=input_data,
          ...
          batch_size=512,
          epochs=22,
          callbacks=[K.callbacks.EarlyStopping(monitor='loss', patience=3, restore_best_weights=True)])
