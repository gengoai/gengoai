...
public abstract class ClassifierEvaluation implements Evaluation, Serializable {
    ...
    protected int getIntegerLabelFor(Observation output, DataSet dataset) {
        if (output.isNDArray() || output.isClassification()) {
            final NumericNDArray y = output.asNumericNDArray();
            if (y.shape().isScalar()) {
        ...
        if (output.isVariable() && dataset.getMetadata(outputName).getEncoder() != null) {
            final Variable v = output.asVariable();
            final Encoder encoder = dataset.getMetadata(outputName).getEncoder();
            return encoder.encode(v.getName());
        }
