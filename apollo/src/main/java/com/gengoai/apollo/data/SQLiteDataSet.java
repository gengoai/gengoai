...
/**
 * <p>A disk-based DataSet that is backed using an SQLite Database.</p>
 *
...
public final class SQLiteDataSet extends DataSet {
   ...
   /**
    * Instantiates a new SQLiteDataSet with data stored in a temporary file.
    *
    * @param stream the stream of datum to add to the dataset
    */
   @SneakyThrows
   ...
   /**
    * Instantiates a new SQLiteDataSet with data stored in the given <code>location</code>.
    *
   ...
   /**
    * Instantiates a new SQLiteDataSet loading data if available from the given <code>location</code>.
    *
    * @param location the location of the DataSet
    */
   @SneakyThrows
   public SQLiteDataSet(@NonNull Resource location) {
      this.executor = SQLContext.create(
            SQLiteConnectionRegistry.getConnection("jdbc:sqlite:"
                                                         + Strings.prependIfNotPresent(location.path(), "/")),
            new SQLiteDialect());
      initDB();
   }
   ...
   public DataSet map(@NonNull SerializableFunction<? super Datum, ? extends Datum> function) {
      ...
      try (NamedPreparedStatement preparedStatement =
                 new NamedPreparedStatement(connection,
                                            executor.render(dataTable.update().set(json, SQL.namedArgument("json"))
                                                                     .where(SQL.C("rowid")
   ...
   @SneakyThrows
   private MStream<Tuple2<Long, Datum>> parallelIdStream() {
