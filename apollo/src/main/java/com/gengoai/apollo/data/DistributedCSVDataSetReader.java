 ...
 * represents {@link Datum} and columns {@link com.gengoai.apollo.data.observation.Observation}s. Generated {@link Datum}
...
public class DistributedCSVDataSetReader implements DataSetReader {
   ...
   public DataSet read(@NonNull Resource dataResource) throws IOException {
      ...
      org.apache.spark.sql.Dataset<Row> rows = sqlContext.read()
                                                         .option("delimiter", Character.toString(csv.getDelimiter()))
                                                         .option("escape", Character.toString(csv.getEscape()))
                                                         .option("quote", Character.toString(csv.getQuote()))
                                                         .option("comment", Character.toString(csv.getComment()))
