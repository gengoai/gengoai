...
public class MiniBatchKMeans extends FlatCentroidClusterer {
   ...
   public void estimate(@NonNull DataSet dataset) {
      ...
      final List<NumericNDArray> vectors = dataset.parallelStream()
                                                  .map(this::getNDArray)
                                                  .collect();
      PrimitiveIterator.OfInt itr = random.ints(0, vectors.size())
                                          .distinct()
                                          .limit(parameters.K.value())
                                          .iterator();
   ...
   private double iteration(List<NumericNDArray> stream, int[] counts, int batchSize) {
      ...
      List<Tuple3<NumericNDArray, Integer, Double>> batch = random.ints(0, (int) stream.size())
                                                                  .limit(batchSize)
                                                                  .parallel()
                                                                  .mapToObj(i -> best(stream.get(i))
                                                                        .appendLeft(stream.get(i)))
