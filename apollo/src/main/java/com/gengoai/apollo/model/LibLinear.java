...
public class LibLinear extends SingleSourceModel<LibLinear.Parameters, LibLinear> {
    ...
    public NumericNDArray getWeightParameters() {
        double[] weights = model.getFeatureWeights();
        NumericNDArray ndArray = nd.DFLOAT32.zeros(Shape.shape(model.getNrFeature(), model.getNrClass() - 1));
        for (int i = 0; i < model.getNrFeature(); i += model.getNrClass()) {
            for (int j = 0; j < model.getNrClass() - 1; j++) {
                ndArray.set(i, j, weights[i + j]);
            }
        }
        return ndArray;
    }

    public Table<String, String, Double> getWeightParameters(Encoder featureEncoder, Encoder labelEncoder) {
        Table<String, String, Double> parameterMatrix = new HashBasedTable<>();
        NumericNDArray ndArray = getWeightParameters();
        for (int row = 0; row < ndArray.rows(); row++) {
            String f = featureEncoder.decode(row);
            if (row >= featureEncoder.size() || f.equalsIgnoreCase("UNKNOWN")) {
                f = "**BIAS**";
            }
            for (int col = 0; col < ndArray.columns(); col++) {
                if (ndArray.get(row, col).doubleValue() != 0) {
                    parameterMatrix.put(f, labelEncoder.decode(col), ndArray.get(row, col).doubleValue());
                }
            }
        }
