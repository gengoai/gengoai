...
public class TFVarSpec implements Serializable {
   ...
   public Shape createBatchShape(int batch_size, @NonNull int... dimensions) {
      int[] batch_shape = new int[shape.length + 1];
      batch_shape[0] = batch_size;
      for (int i = 0; i < shape.length; i++) {
         if (shape[i] < 0) {
      ...
      return Shape.shape(batch_shape);
   }

   public NDArray encode(Shape shape, NDArray x) {
      if (shape.channels() > 0) {
         return x.padPost(Shape.ROW, shape.rows(),
                          Shape.COLUMN, shape.columns());
      }
      return x.T().padPost(Shape.COLUMN, shape.columns());
   }
   ...
   public void updateBatch(NDArray batch, int index, NDArray x) {
      ...
      if (batch.shape().channels() > 0) {
         batch.setSlice(index, x.padPost(Shape.ROW, shape.rows(),
                                         Shape.COLUMN, shape.columns()));
      } else {
