...
import static com.gengoai.apollo.model.sequence.Order.FIRST;
...
public class MalletCrf extends SingleSourceModel<MalletCrf.Parameters, MalletCrf> {
   ...
   public void estimate(DataSet preprocessed) {
      ...
      for(Datum datum : preprocessed) {
         com.gengoai.apollo.data.observation.Sequence<?> x = datum.get(parameters.input.value()).asSequence();
         com.gengoai.apollo.data.observation.Sequence<?> y = datum.get(parameters.output.value()).asSequence();
