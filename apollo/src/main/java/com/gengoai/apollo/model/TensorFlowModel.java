...
public abstract class TensorFlowModel implements Model {
   ...
   protected Map<String, Tensor<?>> createTensors(DataSet batch) {
      ...
      inputs.forEach((name, spec) -> {
         final var batchShape = spec.createBatchShape(batch_size, max_sequence_length);
         List<NDArray> arrays = batch.stream()
                                     .map(datum -> spec.encode(batchShape, datum.get(name).asNDArray()))
                                     .collect(Collectors.toList());
         NDArray n = batchShape.isTensor()
               ? nd.dstack(arrays)
               : nd.vstack(arrays);
         tensors.put(spec.servingName, n.toTensor());
      });
      return tensors;
   }
