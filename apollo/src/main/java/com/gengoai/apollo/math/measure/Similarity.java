...
public enum Similarity implements SimilarityMeasure {
   ...
   DiceGen2 {
      ...
      public double calculate(@NonNull ContingencyTable table) {
         Validation.checkArgument(table.rowCount() == table.columnCount() && table.rowCount() == 2,
                                  "Only supports 2x2 contingency tables.");
         return  table.get(0, 0) / (table.columnSum(0) + table.rowSum(0));
      }
   },
   /**
   ...
   Angular {
      @Override
      public double calculate(@NonNull ContingencyTable table) {
         return 1.0 - 2 * Math.acos(Cosine.calculate(table)) / Math.PI;
      }

      @Override
      public double calculate(@NonNull NumericNDArray v1, @NonNull NumericNDArray v2) {
         return 1.0 - 2 * Math.acos(Cosine.calculate(v1, v2)) / Math.PI;
      }
