 ...
 */
public class SparseFloat64NDArray extends DoubleNDArray {
   ...
   protected SparseFloat64NDArray(Shape shape) {
      ...
      for (int i = 0; i < shape.sliceLength(); i++) {
         this.data[i] = new OpenIntDoubleHashMap();
      }
   }

   @JsonCreator
   protected SparseFloat64NDArray(@JsonProperty("data") double[] data,
   ...
   public NumericNDArray compact() {
      for (OpenIntDoubleHashMap datum : data) {
         datum.trimToSize();
      }
      return this;
   }
   ...
   public NumericNDArray reshape(@NonNull Shape newShape) {
      ...
      SparseFloat64NDArray temp = new SparseFloat64NDArray(newShape);
   ...
   @Override
   public long size() {
      return Arrays.stream(data).mapToLong(OpenIntDoubleHashMap::size).sum();
   }

   @Override
   public NumericNDArray slice(int index) {
      SparseFloat64NDArray v = new SparseFloat64NDArray(Shape.shape(shape().rows(),
                                                                    shape().columns()));
      if (data.length == 1) {
         v.data[0] = data[0];
      } else {
   ...
   public NumericNDArray slice(int startKernel, int startChannel, int endKernel, int endChannel) {
      ...
      SparseFloat64NDArray v = new SparseFloat64NDArray(os);
